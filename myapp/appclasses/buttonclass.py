...
class MyButton(object):
	...
	def create_obj(self):
		...
		butt = ctk.CTkButton(self.master, text=self.text,text_color=self.text_color,fg_color=self.fg_color, image = self.image, bg_color=self.bg_color, height=self.height, width=self.width,
			corner_radius=5,border_spacing=0,command=self.command,font=(self.font,self.font_size,self.font_weight))
